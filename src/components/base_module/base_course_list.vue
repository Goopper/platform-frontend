  ...
  <div style="background-color: white;border: 1px #e3e3e3 solid;width: 16%;">
    <v-list open-strategy="multiple" active-strategy="single-independent" style="padding: 0;">
      <v-list-item style="border: 1px #e3e3e3 solid;">
        <v-list-item-title @click="detail">
          JAVA
        </v-list-item-title>
      </v-list-item>
      <v-list-group v-for="chapter in data" :key="chapter.id" :value="chapter.id" subgroup>
        <template v-slot:activator="{ props }">
          <v-list-item :title="chapter.name" v-bind="props" :to="`/course/chapter/${chapter.id}`">
          </v-list-item>
        </template>
        <v-list-item class="tile" v-for="task in chapter.tasks" :key="task.id" :title="task.name" :value="task.name" :to="`/course/task/${task.id}`">
        </v-list-item>
      </v-list-group>
    </v-list>
  </div>
